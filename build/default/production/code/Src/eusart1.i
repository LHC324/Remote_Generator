...
# 2 "<built-in>" 2
# 1 "code/Src/eusart1.c" 2
# 50 "code/Src/eusart1.c"
# 1 "code/Inc\\eusart1.h" 1
# 54 "code/Inc\\eusart1.h"
...
_Bool EUSART1_is_rx_ready(void) {
    return (Uart1Rx.eusart1RxCount ? 1 : 0);
}

_Bool EUSART1_is_tx_done(void) {
    return TXSTA1bits.TRMT;
}
# 162 "code/Src/eusart1.c"
void EUSART1_Write(uint8_t txData) {
...
void EUSART1_Receive_ISR(void) {
    ...
    if (RCSTA1bits.FERR) {
        ...
        Uart1Rx.rxstatus.ferr = 1;
        EUSART1_FramingErrorHandler();
    }

    if (RCSTA1bits.OERR) {

        Uart1Rx.rxstatus.oerr = 1;
        EUSART1_OverrunErrorHandler();
    }


    if (Uart1Rx.rxstatus.status) {
...
void EUSART1_RxDataHandler(void) {
    ...
    if (Uart1Rx.eusart1RxCount <= 32 ) {

        Uart1Rx.eusart1RxBuffer[Uart1Rx.eusart1RxCount++] = RCREG1;
    }
}
...
uint8_t Uart1_Send(uint8_t* dat, uint8_t len) {
    uint16_t temp_len = 0;
    ...
    if (EUSART1_is_tx_ready()) {
        ...
        eusart1TxBufferRemaining -= temp_len;
        memcpy((uint8_t *)eusart1TxBuffer, dat, temp_len);
        PIE1bits.TX1IE = 1;
    } else {
        return 0xFF;
    }
