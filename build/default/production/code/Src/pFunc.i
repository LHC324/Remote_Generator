...
# 13 "code/Src/pFunc.c" 2
# 157 "code/Src/pFunc.c"
...
float kalmanFilter(KFP *kfp, uint32_t input) {
    ...
    kfp->Output = kfp->Output + kfp->Kg * (input - kfp->Output);
...
_Bool checkCrc16(uint8_t *ptr, uint8_t length) {
    ...
    crc2 = (uint16_t)*(ptr + length - 1U) << 8U;
    crc2 += (uint16_t)*(ptr + length - 2U);
