...
# 2 "<built-in>" 2
# 1 "code/Src/eusart2.c" 2
# 50 "code/Src/eusart2.c"
# 1 "code/Inc\\eusart2.h" 1
# 54 "code/Inc\\eusart2.h"
...
_Bool EUSART2_is_rx_ready(void) {
    return (Uart2Rx.eusart2RxCount ? 1 : 0);
}

_Bool EUSART2_is_tx_done(void) {
    return TXSTA2bits.TRMT;
}
# 164 "code/Src/eusart2.c"
void EUSART2_Write(uint8_t txData) {
...
void EUSART2_Receive_ISR(void) {
    ...
    if (RCSTA2bits.FERR) {
        ...
        Uart2Rx.rxstatus.ferr = 1;
        EUSART2_FramingErrorHandler();
    }

    if (RCSTA2bits.OERR) {

        Uart2Rx.rxstatus.oerr = 1;
        EUSART2_OverrunErrorHandler();
    }


     if (Uart2Rx.rxstatus.status) {
...
void EUSART2_RxDataHandler(void) {
       ...
       if (Uart2Rx.eusart2RxCount <= 64 ) {
        Uart2Rx.eusart2RxBuffer[Uart2Rx.eusart2RxCount++] = RCREG2;
    }
}
...
uint8_t Uart2_Send(uint8_t* dat, uint8_t len) {
    uint8_t temp_len = 0;
    ...
    if (EUSART2_is_tx_ready()) {
        eusart2TxTail = 0;
        PIE3bits.TX2IE = 0;
        if (len <= sizeof (eusart2TxBuffer)) {
            temp_len = len;
        } else {
            temp_len = sizeof (eusart2TxBuffer);
        }
        eusart2TxHead = temp_len;
        eusart2TxBufferRemaining -= temp_len;
        memcpy((uint8_t *)eusart2TxBuffer, dat, eusart2TxHead);
        PIE3bits.TX2IE = 1;
    } else {
        return 0xFF;
    }

    return EUSART2_is_tx_done();
}
